...
| 场景 | 操作 |
|------|------|
| **首次使用** | 1. 三个桌面端全开并登录<br>2. 网关面板点「同步已勾选」写入 Hermes<br>3. 客户端填三个 Base URL + 留空 API Key |
| **日常使用** | 三个桌面端后台常驻（托盘最小化即可） |
| **CodeArts 会话过期** | 网关面板点「授权」→ 浏览器自动打开 → 登录同意 → 自动续期 |
| **MonkeyCode 单独用** | 面板填 `mc_api_key` + `mc_base_url`，可关桌面端 |
| **CodeArts 502** | 桌面端未登录/会话过期 → 打开桌面端登录/点网关「授权」按钮 |
| **更新/重装桌面端** | 重新登录一次，网关自动读取新 token |
...
<function=terminal>
<parameter=command>
cd "C:/Users/ASUS/AppData/Local/Temp/bdz-push" && cat > README.md << 'ENDOFREADME'
# BuddyZ Gateway
